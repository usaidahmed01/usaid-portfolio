...
export function Header() {
  return <header className="site-header"><div className="shell nav-shell">
    ...
    <Link className="nav-cta" href="/contact">Start a project <Arrow diagonal /></Link>
    <details className="mobile-menu"><summary aria-label="Open navigation"><span /><span /></summary><nav aria-label="Mobile navigation">{nav.map((item) => <Link key={item.href} href={item.href}>{item.label}</Link>)}<Link href="/contact">Start a project <Arrow /></Link></nav></details>
  </div></header>;
}

export function Footer() {
  return <footer className="footer"><div className="shell">
    <div className="footer-main"><p className="kicker"><span className="status-dot" /> Available for select AI projects</p><h2>Have an ambitious AI idea?</h2><Link className="footer-email" href={`mailto:${site.email}`}>{site.email} <Arrow diagonal /></Link></div>
