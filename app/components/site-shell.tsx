...
export function Footer() {
  return <footer className="footer"><div className="shell">
    ...
    <div className="footer-grid"><div><span className="footer-label">Navigate</span>{nav.map((item) => <Link key={item.href} href={item.href}>{item.label}</Link>)}<Link href="/build-log">Founder build log</Link><Link href="/ai-opportunity-assessment">AI assessment</Link></div><div><span className="footer-label">Connect</span><a href={site.linkedin} target="_blank" rel="noreferrer">LinkedIn <Arrow diagonal /></a><a href={site.github} target="_blank" rel="noreferrer">GitHub <Arrow diagonal /></a></div><div><span className="footer-label">Based in</span><p>{site.location}<br />Working globally</p></div></div>
