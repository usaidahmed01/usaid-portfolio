...
export default function sitemap(): MetadataRoute.Sitemap {
  const staticRoutes = ["", "/work", "/services", "/about", "/insights", "/contact"].map((path) => ({ url: `${base}${path}`, lastModified: new Date(), changeFrequency: path === "/insights" ? "weekly" as const : "monthly" as const, priority: path === "" ? 1 : .8 }));
  ...
  const insightRoutes = insights.map((item) => ({ url: `${base}/insights/${item.slug}`, lastModified: new Date(item.date), changeFrequency: "monthly" as const, priority: .7 }));
