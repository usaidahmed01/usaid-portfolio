...
export default function BlogPage() {
  return <><Header /><main><PageIntro eyebrow="Weekly blog" title="Notes from inside the build." description="Practical writing on AI products, agentic systems, retrieval, machine learning, product architecture, and the difficult path from demonstration to dependable use." />
    ...
    <section className="listing-section shell"><div className="posts-list">{insights.map((post, index) => <Link href={`/blog/${post.slug}`} key={post.slug} className="post-row"><span>0{index + 1}</span><div><p>{post.category}</p><h2>{post.title}</h2><small>{post.date} · {post.readingTime}</small></div><Arrow diagonal /></Link>)}</div></section>
